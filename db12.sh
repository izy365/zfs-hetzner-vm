...
# Pre-configure automatic service restarts
echo "libc6 libraries/restart-without-asking boolean true" | debconf-set-selections
echo "libc6:amd64 libraries/restart-without-asking boolean true" | debconf-set-selections
echo "libssl3 libraries/restart-without-asking boolean true" | debconf-set-selections
...
function check_prerequisites {
  ...
  if ! dpkg-query --showformat="\${Status}" -W dialog 2> /dev/null | grep -q "install ok installed"; then
    DEBIAN_FRONTEND=noninteractive apt install --yes dialog
  fi
}
...
function chroot_execute {
  DEBIAN_FRONTEND=noninteractive chroot $c_zfs_mount_dir bash -c "$1"
}
...
# Set configuration for automatic service restarts during upgrades
echo "libc6 libraries/restart-without-asking boolean true" | debconf-set-selections
echo "libc6:amd64 libraries/restart-without-asking boolean true" | debconf-set-selections
echo "libssl3 libraries/restart-without-asking boolean true" | debconf-set-selections
echo "libssl3:amd64 libraries/restart-without-asking boolean true" | debconf-set-selections
...
for kver in $(find /lib/modules/* -maxdepth 0 -type d | grep -v "$(uname -r)" | cut -s -d "/" -f 4); do
  DEBIAN_FRONTEND=noninteractive apt purge --yes "linux-headers-$kver"
  DEBIAN_FRONTEND=noninteractive apt purge --yes "linux-image-$kver"
done

echo "======= installing zfs on rescue system =========="

echo "zfs-dkms zfs-dkms/note-incompatible-licenses note true" | debconf-set-selections  
# Pre-seed debconf to avoid prompts
DEBIAN_FRONTEND=noninteractive apt install --yes software-properties-common dpkg-dev dkms
...
apt update  
DEBIAN_FRONTEND=noninteractive apt install -t testing --yes zfs-dkms zfsutils-linux
...
# Configure the target system to use non-interactive mode
mkdir -p "$c_zfs_mount_dir/etc/apt/apt.conf.d"
echo 'DPkg::options { "--force-confdef"; "--force-confold"; }' > "$c_zfs_mount_dir/etc/apt/apt.conf.d/70debconf"
echo 'Dpkg::Progress-Fancy "0";' > "$c_zfs_mount_dir/etc/apt/apt.conf.d/99progressbar"

# Pre-configure automatic service restarts in the chroot environment
echo "libc6 libraries/restart-without-asking boolean true" > "$c_zfs_mount_dir/tmp/preseed.conf"
echo "libc6:amd64 libraries/restart-without-asking boolean true" >> "$c_zfs_mount_dir/tmp/preseed.conf"
echo "libssl3 libraries/restart-without-asking boolean true" >> "$c_zfs_mount_dir/tmp/preseed.conf"
echo "libssl3:amd64 libraries/restart-without-asking boolean true" >> "$c_zfs_mount_dir/tmp/preseed.conf"
echo "libpam0g libraries/restart-without-asking boolean true" >> "$c_zfs_mount_dir/tmp/preseed.conf"
echo "libpam0g:amd64 libraries/restart-without-asking boolean true" >> "$c_zfs_mount_dir/tmp/preseed.conf"
chroot_execute "debconf-set-selections /tmp/preseed.conf"
...
# Set up locale and keyboard configuration non-interactively
cat > "$c_zfs_mount_dir/tmp/preseed_locale.conf" << CONF
...
chroot_execute "debconf-set-selections /tmp/preseed_locale.conf"
chroot_execute "dpkg-reconfigure locales -f noninteractive"
echo -e "LC_ALL=en_US.UTF-8\nLANG=en_US.UTF-8\n" >> "$c_zfs_mount_dir/etc/environment"
chroot_execute "DEBIAN_FRONTEND=noninteractive apt install --yes keyboard-configuration console-setup"
chroot_execute "DEBIAN_FRONTEND=noninteractive dpkg-reconfigure keyboard-configuration"
chroot_execute "DEBIAN_FRONTEND=noninteractive dpkg-reconfigure console-setup"
chroot_execute "setupcon"

chroot_execute "rm -f /etc/localtime /etc/timezone"
chroot_execute "DEBIAN_FRONTEND=noninteractive dpkg-reconfigure tzdata"

echo "======= installing latest kernel============="
# linux-headers-generic linux-image-generic
chroot_execute "DEBIAN_FRONTEND=noninteractive apt install --yes linux-image${v_kernel_variant}-amd64 linux-headers${v_kernel_variant}-amd64 dpkg-dev"
...
if [[ $v_zfs_experimental == "1" ]]; then
  ...
  chroot_execute "apt update"
  chroot_execute "DEBIAN_FRONTEND=noninteractive apt install -t zfs-debian-experimental --yes zfs-initramfs zfs-dkms zfsutils-linux"
else
  chroot_execute "DEBIAN_FRONTEND=noninteractive apt install -t bookworm-backports --yes zfs-initramfs zfs-dkms zfsutils-linux"
fi
...
chroot_execute "DEBIAN_FRONTEND=noninteractive dpkg-reconfigure openssh-server"
...
for disk in ${v_selected_disks[@]}; do
  chroot_execute "DEBIAN_FRONTEND=noninteractive grub-install --recheck $disk"
done
...
chroot_execute "DEBIAN_FRONTEND=noninteractive apt purge cryptsetup* --yes"
...
chroot_execute "DEBIAN_FRONTEND=noninteractive update-initramfs -u -k all"

chroot_execute "DEBIAN_FRONTEND=noninteractive apt remove cryptsetup* --yes"

echo "======= update grub =========="
chroot_execute "DEBIAN_FRONTEND=noninteractive update-grub"
